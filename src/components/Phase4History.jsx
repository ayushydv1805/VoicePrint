export default function Phase4History({user,history,onRefresh,onOpenDetails}){
  return <div className="page">
    ...
    <div className="panel history-panel">
      ...
      <div className="history-list">{history.map((event)=><div className="history-row" key={event.id}>
        ...
        <button className="ghost-btn history-detail-btn" onClick={() => onOpenDetails?.(event.id)}>View details</button>
      </div>)}</div>}
    </div>
  </div>;
}
