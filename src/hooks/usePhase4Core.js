...
export default function usePhase4Core(){
  ...
  useEffect(()=>{if(!open||!event?.id||event.status==="cancelled"||event.status==="dispatched"||!location||Date.now()-last.current<5000)return;last.current=Date.now();api("/api/v1/sos/events/"+encodeURIComponent(event.id)+"/location",{method:"POST",body:JSON.stringify({location})}).then(r=>setEvent(r.event)).catch(e=>setError(e.message))},[open,event?.id,location]);

  const closeFlow=useCallback(async()=>{if(event?.id&&event.status!=="dispatched"&&event.status!=="cancelled"){try{const r=await api("/api/v1/sos/events/"+encodeURIComponent(event.id)+"/cancel",{method:"POST"});setEvent(r.event);await refresh()}catch(e){setError(e.message)}}if(sosWatch)stopWatching();setSosWatch(false);setOpen(false)},[event,sosWatch,stopWatching,refresh]);
